...
%     end 
    ...
    U = U_dist(i);
    [y1(k,i), y2(k,i), y3(k,i), dist(k,i)] = Coordinates_definition(inputs,twist,AoA,Ra,Psi,Mbc0Te(k,i,:));
   ...
   [S_pp_total(i,:)] = subtract_AtmA(S_pp_total(i,:),a,dist(k,i));
end
...
legend('show','Numcolumns',1,'location','Best','Interpreter','latex','FontSize',26);
grid on
saveas(gcf,['../4 - Results/' File_out 'db.fig']);
saveas(gcf,['../4 - Results/' File_out 'db.png']);
saveas(gcf,['../4 - Results/' File_out 'db.eps'],'epsc');


%[sortedData,Fc,Flow,Fhigh] = NarrowToNthOctave(f,10*log10(4*pi*S_pp_rev),3);
...
figure(4)
semilogx(Fc,Lp_onethird_dbA+6,'-','linewidth',3,'DisplayName',['Uniform'])
hold on
%semilogx(Fc,Lp_onethird_dbA-A+6,'-','linewidth',3,'DisplayName',['Uniform'])
...
legend('show','Numcolumns',1,'location','Best','Interpreter','latex','FontSize',26);
grid on
saveas(gcf,['../4 - Results/' File_out 'One_third_dbA.fig']);
saveas(gcf,['../4 - Results/' File_out 'One_third_dbA.png']);
saveas(gcf,['../4 - Results/' File_out 'One_third_dbA.eps'],'epsc');
save('S_pp_blade_steady_NoTower.mat','S_pp_blade')
save('S_pp_rev_steady_NoTower.mat','S_pp_rev')
toc
